...
document.addEventListener("DOMContentLoaded", function (event) {
	if ((captcha_required = true)) {
		if (document.getElementById("captcha")) {
			...
			captcha_field.onfocus = function () {
				//if tabbing through fields
				if (captcha.src == location.href || captcha.src == "") {
					//if empty, yes this is weird it goes to href when emptied out by js, but '' if never changed before.
					document.querySelector("details.js-captcha").open = true
					create_captcha()
...
document.addEventListener("DOMContentLoaded", function (event) {
	if (document.getElementById("post_password")) {
		//only when post-form is on
...
//expand images
document.addEventListener("DOMContentLoaded", function (event) {
	if (document.querySelector("body.index") || document.querySelector("body.thread")) {
		//only on index/thread
	...
	if (document.querySelector("body.index") || document.querySelector("body.thread")) {
		...
		for (const audio of audios) {
			...
			thumb.addEventListener("click", (event) => {
				...
				fileinfo.insertAdjacentHTML("beforeend", closebutton)
				document
					.querySelector(`span.closeaudio a[closeaudio-id="${audioid}"]`)
...
document.addEventListener("DOMContentLoaded", function (event) {
	if (document.querySelector("body.index") || document.querySelector("body.thread")) {
		//only on index/thread. this can probably be multipurposed later for things like a settings menu
